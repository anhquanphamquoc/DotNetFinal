...
namespace WebApp01.Controllers
{
    ...
    public class CartController : Controller
    {
        ...
        public IActionResult Create()
        {
            ...
            List<CartItemModel> cartItems = HttpContext.Session.GetJson<List<CartItemModel>>("Cart") ?? new List<CartItemModel>();
            List<string> productList = cartItems.Select(item => $"{item.ProductName} - {item.Quantity}").ToList();
            string productString = string.Join(", ", productList);

            // Lưu thông tin sản phẩm vào TempData
            TempData["ProductsName"] = productString;

            // Chuyển hướng tới Create action trong CheckoutController
